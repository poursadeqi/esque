...
def test_message_serializer(
    binary_messages: List[BinaryMessage], string_messages: List[OutputMessage], string_serializer: StringSerializer
):
    serializer: MessageSerializer = MessageSerializer(key_serializer=string_serializer)
    deserialized_message: OutputMessage = serializer.deserialize(binary_messages[0])
...
def test_message_serializer_many(
    ...
    serializer: MessageSerializer = MessageSerializer(key_serializer=string_serializer)
    deserialized_messages: List[OutputMessage] = list(serializer.deserialize_many(binary_messages))
