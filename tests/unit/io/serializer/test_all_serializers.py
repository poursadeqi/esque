...
def test_serialize_many_no_data(serializer: DataSerializer, no_data: PrintableMessagePayload):
    actual_serialized_data: List = list(serializer.serialize_many([no_data]))
    assert actual_serialized_data == [None]

    actual_deserialized_data: List[PrintableMessagePayload] = list(serializer.deserialize_many([None]))
    assert actual_deserialized_data == [no_data]


@parametrize_with_cases("serializer")
def test_serialize_no_data(serializer: DataSerializer, no_data: PrintableMessagePayload):
    actual_serialized_data: None = serializer.serialize(no_data)
    assert actual_serialized_data is None

    actual_deserialized_data: PrintableMessagePayload = serializer.deserialize(None)
    assert actual_deserialized_data == no_data
